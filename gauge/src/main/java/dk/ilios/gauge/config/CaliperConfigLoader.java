    ...
    public GaugeConfiguration loadOrCreate() throws InvalidConfigurationException {
        ...
        if (configFile.exists()) {
            try {
                ...
                return new GaugeConfiguration(mergeProperties(options.configProperties(), user, defaults));
            } catch (IOException keepGoing) {
            }
        }

        ByteSource supplier = Util.resourceSupplier(GaugeConfiguration.class, "/default-config.properties");
