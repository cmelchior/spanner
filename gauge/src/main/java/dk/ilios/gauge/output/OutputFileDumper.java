 ...
 * {@code /sdcard/gauge/results/[benchmark classname].[timestamp].json}; if it exists and is a file,
...
public final class OutputFileDumper implements ResultProcessor {
    ...
    public OutputFileDumper(Run run,
                            ...
                            Gson gson,
                            GaugeConfig gaugeConfig) throws InvalidConfigurationException {
        this.run = run;
        if (gaugeConfig.getResultsFolder() == null) {
            throw new IllegalStateException("Result folder must be specified");
        }

        this.resultFile = new File(new File(gaugeConfig.getResultsFolder(), RESULTS_DIR), createFileName(benchmarkClass.name()));
//        logger.fine("found no configuration");

//        ResultProcessorConfig config = gaugeConfig.getResultProcessorConfig(OutputFileDumper.class);
